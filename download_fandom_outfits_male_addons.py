...
OUT_DIR = Path(os.environ.get("USERPROFILE", str(Path.home()))) / "Desktop" / "TibiaSprites" / "fandom_outfits_male_addons"
...
def pick_image_url_from_cell(td) -> str | None:
    # Fandom often uses <img data-src="..."> or <img src="...">
    img = td.find("img")
    if not img:
        return None
...
def find_outfits_table_and_male_addons_index(soup: BeautifulSoup):
    """
    Find the table that contains a header with 'male addons' (case/spacing tolerant).
    Determine the column index by scanning the first row that contains THs.
    """
    for table in soup.find_all("table"):
        # find a header row containing THs
        ...
        for tr in header_rows:
            ...
            if any("male addons" in h or "male add-ons" in h for h in headers):
                # determine index based on ALL cells in that row (th/td mix)
                cells = tr.find_all(["th", "td"])
                idx = None
                for i, cell in enumerate(cells):
                    h = norm(cell.get_text(" ", strip=True))
                    if "male addons" in h or "male add-ons" in h:
                        idx = i
                        break
                return table, idx
    return None, None
...
def main():
    ...
    table, male_idx = find_outfits_table_and_male_addons_index(soup)
    if not table or male_idx is None:
        raise RuntimeError("Could not find Outfits table with a 'Male Addons' column.")

    results = []
    seen = set()

    rows = table.find_all("tr")
    # skip rows until we pass the header row
    passed_header = False
    ...
    for tr in rows:
        ...
        if len(cells) <= male_idx:
            continue

        # Name is generally first cell text or its first link
        name_cell = cells[0]
        a = name_cell.find("a")
        ...
        male_cell = cells[male_idx]
        img_url = pick_image_url_from_cell(male_cell)
        if not img_url:
            continue
    ...
    print(f"Found outfits with male addons image: {len(results)}")
